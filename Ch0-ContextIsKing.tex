...
Eventually, a biological system with demographic fluctuations and no immigration will eventually go extinct. 
Mathematically this is equivalent to saying that if there is no mechanism to leave a population of zero then extinction, having zero population, is an absorbing state, and the only physically realistic steady state of the system. 
There is a possibility of reaching some other absorbing state but it is unclear what this state might be, since there is always the possibility of death of an organism. 
