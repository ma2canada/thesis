...
\begin{itemize}
	...
	\item $a_{12}=a_{21}=1$ limit recovers Moran results\footnote{Lin, Kim, and Doering. \emph{J. Stat. Phys.}, 2012.} $\tau \sim K$ %\emph{Features of Fast Living: On the Weak Selection for Longevity in Degenerate Birth-Death Processes} 
\end{itemize}
\pause
\begin{columns}
	\begin{column}{5cm}
		\vfill%does not seem to work as I intend (make the figure one line lower)
		${}$ \\
		\begin{center}
...
\begin{column}{6cm}
\begin{center}
\includegraphics[width=\textwidth]{fiftyfifty-exttimevK.png}
\end{center}
\end{column}
\begin{column}{6cm}
\begin{center}
\includegraphics[width=\textwidth]{fiftyfifty-exttimeva.png}
\end{center}
\end{column}
\end{columns}
\emph{Time of a failed invasion.}
\emph{Left:} Solid lines are the numerical results, from $a=0$ mostly being fastest to $a=1$ being slowest, for large $K$. The blue dashed line is Moran result. 
...
%I count ~27 slides [pauses make it look like more]
