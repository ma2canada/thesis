...
\setbeamertemplate{note page}[plain] %to print it out with notes
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%\iffalse
%%EDIT:::still needs work - picture of gut biome, microfluidics; move applications to end
%\begin{frame}
...
\begin{columns}[t]
	\begin{column}{4.2cm}
		\begin{center}
			...
			\visible<4->{
			\vfill
			\normalsize{Conservation Efforts}\\
...
\begin{column}{4.2cm}
	\begin{center}
		\visible<3->{
		%Microfluidics\\
		Small Populations\\%\footnote{Ingalls?}
		%\hspace{-0.6cm}
		%a testable setup \\
		microfluidics \\
...
\mbox{}\hfill\raisebox{-\height}[0pt][0pt]{\includegraphics[width=.45\linewidth]{GaussianOverlap}}
%\includegraphics[width=0.6\textwidth]{GaussianOverlap}
\end{frame}
...
\begin{itemize}
	...
	\item mean time to extinction: $\tau \sim e^K$ %for logistic it's $\tau \sim e^K/K$
		\note[item]{$K$ is large so tau is long}%<1><1->
\end{itemize}
\end{frame}
...
\pause
\large{
Residence time = $\int_0^{\infty} dt P(s|s^0)=\hat{M}^{-1}_{s,s^0}$ so $\boxed{\hat{M}\vec{T}=-\vec{1}}$. \\%\langle t(s^0)\rangle_s = 
}
\pause
...
\begin{itemize}
	...
	\item $a=1$ recovers Moran result $\tau \sim K$: \textbf{neutral limit}	%\item $a_{12}=a_{21}=1$ recovers Moran results $\tau \sim K$: \textbf{neutral limit}%%\footnote{Lin, Kim, and Doering. \emph{J. Stat. Phys.}, 2012.}
...
%\footnotesize{
	...
	Invasion probability lessens as niche overlap increases. The trend is more stark for large $K$. %which is the deterministic limit
%}
\end{frame}
