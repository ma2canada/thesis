...
\mbox{}\hfill\raisebox{-\height}[0pt][0pt]{\includegraphics[width=.4\linewidth]{GaussianOverlap}}
\vspace*{4.5\baselineskip}
\begin{itemize}
	\pause
	\item \underline{problem}: too many resources, parameters required
	\pause
	\item with stochasticity, mean time to extinction $\tau \sim e^K$ %for logistic it's $\tau \sim e^K/K$
\end{itemize}
\note{
...
\pause
\vspace{0.6cm}
Biodiversity comes from a balance of species exiting (fixation, extinction) and species entering (invasion) the system: what are the timescales? \\%immigration
}
\vspace{0.6cm}
%{{\centering
...
\includegraphics[width=0.9\textwidth]{single-logistic-2}
\end{frame}
...
deterministic logistic equation $\dot{x} = r \, x \, \left(1-\frac{x}{K}\right)$
%\pause
\includegraphics[width=0.5\textwidth]{lattice-fig2} \\
...
$\tau \sim e^K$ \hfill 
\visible<2->{ansatz: $\tau(a,K) = e^{\textcolor{orange}{h(a)}}K^{\textcolor{OliveGreen}{g(a)}}e^{\textcolor{blue}{f(a)}K}$} 
\hfill $\tau \sim K$
\end{center}
\begin{columns}
	\begin{column}{5.1cm}
...
\begin{columns}
	\begin{column}{5.0cm}
		\vspace{-0.3cm}
		\begin{center}
			\includegraphics[width=1.0\textwidth]{invasion-pic}
		\end{center}
	\end{column}
	\begin{column}{5.9cm}
		\begin{itemize}
...
%15 slides [pauses and extras make it look like more]
